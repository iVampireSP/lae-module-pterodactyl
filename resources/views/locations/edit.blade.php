<x-app-layout>
    ...
    <form method="POST" action="{{ route('locations.update', $location->id) }}">
        ...
        <br />
        每内存价格(GB)<input type="text" name="memory_price" value="{{ number_format($location->memory_price, 8) }}" />

        <br />
        每硬盘价格(GB)<input type="text" name="disk_price" value="{{ $location->disk_price }}" />
